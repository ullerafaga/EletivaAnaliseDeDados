...
# exercicio: Compartilhe com a gente um código criado por você com a utilização
#de técnica bootstrapping. Lembre-se de compartilhar um link do github!!!

# bootstraping com função replicate
set.seed(2909) # agora, não vamos mais usar como tarefa mas como execução ponto #a ponto

#primeiro, criar o objeto distNormalSimulacao_exercicio
distNormalSimulacao_exercicio <- rnorm(100) # usa a função rnorm para criar uma
#distribuição normal, indicando o total de casos

bootsDistNormal10_exercicio <- replicate(10, sample(distNormalSimulacao_exercicio, 10, replace = TRUE)) # replicamos 10x a amostra,
#criando assim um bootstrapping
bootsDistNormal10_exercicio

# calculando uma estatística com bootstrapping (10 amostras)
mediaBootsNormal10_exercicio <- replicate(10, mean(sample(distNormalSimulacao, 10, replace = TRUE))) # calculamos a média de 10 amostras de 10 casos
mediaBootsNormal50_exercicio <- replicate(50, mean(sample(distNormalSimulacao, 10, replace = TRUE))) # calculamos a média de 50 amostras de 10 casos
mediaBootsNormal100_exercicio <- replicate(100, mean(sample(distNormalSimulacao, 10, replace = TRUE))) # calculamos a média de 100 amostras de 10 casos
mediaBootsNormal1000_exercicio <- replicate(1000, mean(sample(distNormalSimulacao, 10, replace = TRUE))) # calculamos a média de 1000
#amostras de 10 casos

# comparando as médias
mean(mediaBootsNormal10_exercicio) # media do boostraping 10
mean(mediaBootsNormal50_exercicio) # media do boostraping 50
mean(mediaBootsNormal100_exercicio) # media do boostraping 100
mean(mediaBootsNormal1000_exercicio) # media do boostraping 1000
mean(distNormalSimulacao_exercicio) # media dos dados originais

# observo que a média de 1000 amostras de 10 casos foi a mais próxima da média
#dos dados originais
