...
m_EU_covid <- EU_covid %>% group_by(location) %>% mutate(row = row_number()) %>% select(location, new_cases, row) # cria matriz dos países, agrupando por local, criando uma nova linha com index e selecionando apenas algumas variáveis

# filtra dados para garantir que todos os países tenham mesmo nro de casos
result <- m_EU_covid %>% group_by(location) %>% filter(row == max(row))
m_EU_covid <- m_EU_covid %>% filter(row<=min(result$row)) 

# pivota o data frame de long para wide
m_EU_covidw <- m_EU_covid %>% pivot_wider(names_from = row, values_from = new_cases) %>% remove_rownames %>% column_to_rownames(var="location") 
