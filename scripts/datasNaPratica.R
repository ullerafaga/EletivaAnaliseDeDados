...
# usarei a variável deathsMS

fitLL_exercicio <- drm(deathsMS ~ dia, fct = LL2.5(),
             data = covidPE, robust = 'mean') # fazendo a predição log-log com a função drm

plot(fitLL_exercicio, log="", main = "Log logistic") # observando o ajuste

predLL_exercicio <- data.frame(predicao = ceiling(predict(fitLL_exercicio, predDia))) # usando o modelo para prever para frente, com base no vetor predDia
predLL_exercicio$data <- seq.Date(as.Date('2020-03-12'), by = 'day', length.out = length(predDia$dia)) # criando uma sequência de datas para corresponder aos dias extras na base de predição

predLL_exercicio <- merge(predLL, covidPE, by.x ='data', by.y = 'date', all.x = T) # juntando as informações observadas da base original 

#library(plotly) # biblioteca para visualização interativa de dados

plot_ly(predLL_exercicio) %>% add_trace(x = ~data, y = ~predicao, type = 'scatter', mode = 'lines', name = "Mortes - Predição") %>% add_trace(x = ~data, y = ~deathsMS.x, name = "Mortes", mode = 'lines') %>% layout(
...
covidPE <- covidPE %>% mutate(deathsMS = round(rollmean(x = newCases, 7, align = "right", fill = NA), 2)) # média móvel de 7 dias

covidPE <- covidPE %>% mutate(deathsMS = dplyr::lag(newCases, 7)) # valor defasado em 7 dias

plot_ly(covidPE) %>% add_trace(x = ~date, y = ~deathsMS, type = 'scatter', mode = 'lines', name = "Mortes") %>% add_trace(x = ~date, y = ~deathsMS, name = "Mortes", mode = 'lines') %>% layout(
  title = 'Mortes por COVID19 em Pernambuco', 
  xaxis = list(title = 'Data', showgrid = FALSE), 
  yaxis = list(title = 'Mortes', showgrid = FALSE),
  hovermode = "compare") # plotando tudo junto, para comparação
